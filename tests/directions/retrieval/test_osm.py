"""Tests for OSM area reading and each retrieval version's polygon profile."""

from pathlib import Path
from typing import Any, cast

import fineweb_polygons.directions.retrieval.osm as osm_module
import fineweb_polygons.directions.retrieval.osm as polygons_module
from fineweb_polygons.directions.retrieval.models import PolygonProfile
from fineweb_polygons.directions.retrieval.osm import (
...
def test_read_named_polygon_profiles_disables_location_loading(
    ...
    class FakeHandler:
        ...
        def apply_file(self, *args: object, **kwargs: object) -> None:
            calls.append((args, kwargs))

    monkeypatch.setattr(polygons_module, "_NamedPolygonHandler", FakeHandler)

    result = polygons_module.read_named_polygon_profiles(pbf)

    assert result.profiles == ()
    assert calls == [((str(pbf),), {"locations": False})]
...
def test_materialize_area_marks_a_real_unnamed_area(monkeypatch) -> None:
    area = _FakeArea()
    area.tags = {}
    monkeypatch.setattr(osm_module.osmium.osm, "Area", _FakeArea)
...
def _read_fake_areas(monkeypatch, materializations):
    ...
    monkeypatch.setattr(
        osm_module.osmium, "FileProcessor", lambda path: FakeProcessor()
    )
    monkeypatch.setattr(
        osm_module,
        "_materialize_area",
...
def test_point_in_ring_keeps_the_start_edge_and_strict_ray_boundary() -> None:
    ring = ((0.0, 1.0), (6.0, 2.0), (5.0, 6.0), (1.0, 5.0))

    assert _point_in_ring((1.0, 1.5), ring) is True
    assert _point_in_ring((3.0, 1.5), ring) is False
...
def test_v3_area_selection_distinguishes_non_areas_and_unnamed_areas(
    ...
    monkeypatch.setattr(osm_module.osmium.osm, "Area", _FakeV3Area)

    assert _select_area(object()) == _AreaSelection(False, False, None)
    assert _select_area(_FakeV3Area(None)) == _AreaSelection(True, False, None)
...
def test_v3_area_reader_counts_all_unnamed_areas_and_continues(
    ...
    monkeypatch.setattr(osm_module.osmium, "FileProcessor", _FakeProcessor)
    monkeypatch.setattr(osm_module, "_select_area", lambda area: next(selections))

    result = osm_module.read_v3_polygon_profiles(Path("mini.osm.pbf"))
